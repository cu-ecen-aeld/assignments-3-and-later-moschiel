...
// Receives dada from client and appends to AESD_DATA_FILE, creating this file if it doesn’t exist.
int recv_client_data_and_append_to_file(char *data_buf, size_t buf_len) {
    int data_file = open(AESD_DATA_FILE, O_CREAT | O_WRONLY | O_APPEND, 0644);
    if (data_file == -1) {
...
// Returns the full content of AESD_DATA_FILE to the client as soon as the received data packet completes.
int send_file_data_to_client(char *data_buf, size_t buf_len) {
    int data_file = open(AESD_DATA_FILE, O_RDONLY);
    if (data_file == -1) {
